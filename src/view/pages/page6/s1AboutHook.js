	...
	return (
		<div className="page-layout">
			...
			<h2 className="h2">
				...
				<p className="p">
					データの取得、購読 (subscription) の設定、あるいはReactコンポーネント内のDOMの手動での変更、
					といったものはすべて副作用の例である。これらを “副作用 (side-effect)“（あるいは単に “作用 (effect)“）と呼ぶ。
				</p>
				<p className="p">
					レンダー後に実行したい処理などを実装する際に用いるフックで、「useEffect」が副作用フックと言われるものである。
				</p>
				<p className="p">
					「useEffect」フックはクラスコンポーネントで用いる「componentDidMount」と「componentDidUpdate」と「componentWillUnmount」
					がまとまったものだと考えることができる。
				</p>
				<p className="p">
					詳しくは次セクション<span className="page-jump" onClick={allData[6].children[1].func}>[ {allData[6].children[1].name} ]</span>で解説
				</p>
